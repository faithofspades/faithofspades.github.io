export class LH24FilterNode extends AudioWorkletNode {
  constructor(audioContext, options = {}) {
    ...
    const defaultOptions = {
      ...
      parameterData: {
        ...
        variant: 1.0, // HP cutoff control: 0=16kHz, 0.5=500Hz, 1=8Hz (bypassed)
        currentMidiNote: 69,
    ...
    super(audioContext, 'lh-24-filter-processor', nodeOptions);
  ...
  setAmplitudeGainNode(gainNode) {
    ...
    console.log(`LH-24 Filter ${this._currentMidiNote} using independent envelope (not following amplitude)`);
  }
  _setupEnvelope() {
...
setResonance(value) {
  ...
  setVariant(value) {
    // Variant controls HP cutoff: 0=16kHz (aggressive), 0.5=500Hz, 1=8Hz (bypassed)
    this.parameters.get('variant').value = Math.max(0.0, Math.min(1.0, value));
  }
  ...
  setClassicMode(enabled) {
    ...
    console.log(`LH-24 Filter: Classic mode ${enabled ? 'enabled' : 'disabled'}`);
  }
...
export class PolyphonicMoogFilter {
  ...
  try {
    ...
    if (!this._processorLoaded) {
      ...
      try {
        await this.audioContext.audioWorklet.addModule('./js/lh-24-filter-processor.js');
        console.log("Successfully loaded lh-24-filter-processor.js");
        this._processorLoaded = true;
      } catch (loadError) {
        console.error('Failed to load with ./js/ path, trying relative path...');
        try {
          await this.audioContext.audioWorklet.addModule('lh-24-filter-processor.js');
