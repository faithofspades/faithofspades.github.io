export class LH12FilterNode extends AudioWorkletNode {
    ...
    const defaultOptions = {
      ...
      parameterData: {
        ...
        envelopeAmount: 0.0,
        keytrackAmount: 0.0,
        bassCompensation: 0.5, // Repurposed as HP amount
        currentMidiNote: 69,
        adsrValue: 0.0
      }
    };
    ...
    super(audioContext, 'lh-12-filter-processor', nodeOptions);
  ...
  setAmplitudeGainNode(gainNode) {
    ...
    console.log(`LH-12 Filter ${this._currentMidiNote} using independent envelope (not following amplitude)`);
  }
  ...
  noteOn(midiNote, velocity = 1.0, retrigger = true, envelopeState = 'idle', currentEnvelopeValue = 0, isLegatoTransition = false) {
    ...
    if (isLegatoTransition || !retrigger) {
      console.log(`LH-12 Filter legato transition to note ${midiNote} - maintaining current envelope`);
      ...
      console.log(`LH-12 Filter envelope triggered for note ${midiNote} - Attack: ${this.attackTime.toFixed(3)}s, Decay: ${this.decayTime.toFixed(3)}s, Sustain: ${this.sustainLevel.toFixed(2)}`);
    }
  }
  ...
  noteOff() {
    ...
    console.log(`LH-12 Filter envelope released - Release time: ${this.releaseTime.toFixed(3)}s`);
  }
  ...
  reset(targetValue = null, isVoiceSteal = false) {
    ...
    if (isVoiceSteal && targetValue !== null) {
      ...
      console.log(`LH-12 Filter smoothly transitioning to new ADSR value ${targetValue.toFixed(2)} (voice steal)`);
    } else {
  ...
  setBassCompensation(value) {
    // Repurposed as HP amount - accepts 0.0 to 1.0
  ...
  setEnvelopeAmount(value) {
    ...
    console.log(`LH-12 FilterNode: envelope amount set to ${this._envelopeAmount.toFixed(2)}`);
  }
  ...
  setClassicMode(enabled) {
    ...
    console.log(`LH-12 Filter: Classic mode ${enabled ? 'enabled' : 'disabled'}`);
  }
...
export class PolyphonicLH12Filter {
  constructor(audioContext, options = {}) {
  ...
  async _loadWorkletProcessor() {
    try {
      ...
      if (!this._processorLoaded) {
        try {
          await this.audioContext.audioWorklet.addModule('./js/lh-12-filter-processor.js');
          console.log("Successfully loaded lh-12-filter-processor.js");
          this._processorLoaded = true;
        } catch (loadError) {
          console.error('Failed to load with ./js/ path, trying relative path...');
          try {
            await this.audioContext.audioWorklet.addModule('lh-12-filter-processor.js');
      ...
      console.error('Failed to load LH-12 filter AudioWorklet processor:', error);
  ...
  async noteOn(noteId, midiNote, velocity = 1.0) {
    ...
    // Create filter node using LH12FilterNode
    const filterNode = new LH12FilterNode(this.audioContext, {
      parameterData: {
        cutoff: this.cutoff,
