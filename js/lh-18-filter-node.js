export class LH18FilterNode extends AudioWorkletNode {
...
export class PolyphonicLP12Filter {
  constructor(audioContext, options = {}) {
  ...
  try {
    ...
    if (!this._processorLoaded) {
      ...
      try {
        await this.audioContext.audioWorklet.addModule('./js/lp-12-filter-processor.js');
        console.log("Successfully loaded lp-12-filter-processor.js");
        this._processorLoaded = true;
      } catch (loadError) {
        console.error('Failed to load with ./js/ path, trying relative path...');
        try {
          await this.audioContext.audioWorklet.addModule('lp-12-filter-processor.js');
    ...
    console.error('Failed to load LP-12 filter AudioWorklet processor:', error);
...
async _loadWorkletProcessor() {
  ...
  async noteOn(noteId, midiNote, velocity = 1.0) {
    ...
    // Create filter node using LP12FilterNode instead of MoogFilterNode
    const filterNode = new LP12FilterNode(this.audioContext, {
      parameterData: {
        cutoff: this.cutoff,
